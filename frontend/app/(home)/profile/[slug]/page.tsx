...
export default function ProfilePage({ params }: { params: { slug: string } }) {
	...
	return (
		<>
			<Box className='flex min-h-screen items-start relative p-4 justify-between max-w-4xl'>
				{profileData ? (
					<>
						<Box sx={{ display: 'flex', height: '100%', mb: 3 }}>
							...
							<Box sx={{ ml: 1 }}>
								<Box sx={{ display: 'flex', alignItems: 'center' }}>
									<Typography sx={{ fontSize: 36, mr: 1 }}>
						...
						<Box>
							<Typography sx={{ fontSize: 24 }}>Статистика</Typography>
							<Divider sx={{ mb: 1 }} />
							<Box>
